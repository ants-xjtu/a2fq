...
SimpleSwitch::SimpleSwitch(bool enable_swap, port_t drop_port)
  : Switch(enable_swap),
    ...
    input_buffer(new InputBuffer(
        10 /* normal capacity */, 1024 /* resubmit/recirc capacity */)),
#ifdef SSWITCH_PRIORITY_QUEUEING_ON
    egress_buffers(nb_egress_threads,
                   1024, EgressThreadMapper(nb_egress_threads),
...
#ifdef SSWITCH_PRIORITY_QUEUEING_ON
void
SimpleSwitch::rotate_priority(size_t port_id, size_t queue_id) {
  egress_buffers.rotate_priority(port_id, queue_id);
  BMLOG_DEBUG("Rotate Priority to queue {} for port {}", queue_id, port_id);
}

size_t
SimpleSwitch::get_highest_pri_qid(size_t port_id) const {
  return egress_buffers.get_highest_pri_qid(port_id);
}

size_t
SimpleSwitch::get_queue_length(size_t port_id, size_t queue_id) const {
  return egress_buffers.size(port_id, queue_id);
}
#endif
