...
class rotate_priority : public ActionPrimitive<const Data &, const Data &> {
  void operator ()(const Data &port_id, const Data &queue_id) {
    simple_switch->rotate_priority(port_id.get_uint(), queue_id.get_uint());
  }
};

REGISTER_PRIMITIVE(rotate_priority);

class read_highest_pri : public ActionPrimitive<const Data &, Data &> {
  void operator ()(const Data &port_id, Data &queue_id) {
    size_t qid = simple_switch->get_highest_pri_qid(port_id.get_uint());
    queue_id.set(static_cast<unsigned int>(qid));
  }
};

REGISTER_PRIMITIVE(read_highest_pri);

class get_queue_length : public ActionPrimitive<const Data &, const Data &, Data &> {
  void operator ()(const Data &port_id, const Data &queue_id, Data &qlen) {
    size_t length = simple_switch->get_queue_length(port_id.get_uint(), queue_id.get_uint());
    qlen.set(static_cast<unsigned int>(length));
  }
};

REGISTER_PRIMITIVE(get_queue_length);
#endif
