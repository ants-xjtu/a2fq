...
struct standard_metadata_t {
    ...
    bit<19> deq_qdepth;
    // queue priority of packet, read only at egress control
...
/*
 * Rotate priority: set queue_id to be the queue with highest priority
 *                  for port port_id.
 * Read highest pri: get the queue_id of the highest priority queue
 *                   for port port_id.
 */
extern void rotate_priority(in bit<9> port_id, in bit<5> queue_id);
extern void read_highest_pri(in bit<9> port_id, out bit<5> queue_id);

/*
 * Set port id and queue id, get current queue length.
 */
extern void get_queue_length(in bit<9> port_id, in bit<5> queue_id, out bit<19> qlen);
